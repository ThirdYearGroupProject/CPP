...
public class Feed {
	...
	public void loadFeed() {
		...
		eventLabels = removeUnlikedEvent();
		GWT.log("size:" + eventLabels.size());
	...
	private List<PanelLabel> removeUnlikedEvent() {
		List<PanelLabel> newEventsLabel = new ArrayList<PanelLabel>();
		for (PanelLabel panel : eventLabels) {
			String isLike = panel.getAttributeAsString("indicator") ;
			if (isLike == null || isLike.equals("icons/48/like.png")) {
				newEventsLabel.add(panel);
			}
		}
		return newEventsLabel;
	}
